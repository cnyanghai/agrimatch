...
function goEdit(id: number) {
  uni.navigateTo({ url: `/pages/requirement/edit?id=${id}` })
}
      ...
      >
        ...
        <view class="my-card__content">
          <view class="my-card__top">
            ...
            <text class="my-card__price">{{ formatPrice(item.expectedPrice) }}</text>
          </view>

          <view class="my-card__info">
            <text v-if="item.quantity" class="my-card__tag">{{ item.quantity }}吨</text>
            <text v-if="item.packaging" class="my-card__tag">{{ item.packaging }}</text>
...
.my-card {
  ...
  &__accent {
    width: 6rpx;
    flex-shrink: 0;
    background: linear-gradient(180deg, $autumn-500 0%, $autumn-300 100%);
...
.fab {
  ...
  background: linear-gradient(135deg, $autumn-500 0%, $autumn-400 100%);
  display: flex;
  align-items: center;
  justify-content: center;
  box-shadow: 0 8rpx 28rpx rgba($autumn-400, 0.35);
  transition: transform 0.15s;
  z-index: 20;
