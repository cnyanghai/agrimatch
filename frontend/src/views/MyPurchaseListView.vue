...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-neutral-900">我的采购</h1>
        <p class="text-sm text-neutral-500 mt-1">管理已发布的采购需求</p>
      </div>
        ...
        >
          ...
          <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="text-lg font-bold text-neutral-900">{{ req.categoryName }}</h3>
                <p v-if="req.contractNo" class="text-xs text-neutral-400">{{ req.contractNo }}</p>
              </div>
            </div>
            <span
              :class="[
                ...
                getStatusColor(req.status) === 'amber' ? 'bg-warning-50 text-warning-600' :
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <div class="bg-neutral-50 rounded-xl p-3">
              ...
              <div class="font-bold text-neutral-900">
                {{ req.quantity ?? '-' }} 吨
                <span v-if="req.status === 1 && req.remainingQuantity != null" class="text-xs text-neutral-500 font-medium">
            ...
            <div class="bg-neutral-50 rounded-xl p-3">
              ...
              <div class="font-bold text-neutral-900 truncate">{{ req.purchaseAddress || '—' }}</div>
            </div>
            <div class="bg-neutral-50 rounded-xl p-3">
              <div class="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-neutral-400 mb-1">
                <Clock class="w-3 h-3" />
              ...
              <div class="font-bold text-neutral-900">{{ req.expireTime ? formatDate(req.expireTime) : '长期' }}</div>
            </div>
          </div>

          <!-- 标签 -->
          <div class="flex flex-wrap gap-2 mb-4">
            <span v-if="req.packaging" class="px-2.5 py-1 rounded-full text-xs font-bold bg-white border border-neutral-200 text-neutral-600">
              包装：{{ req.packaging }}
            </span>
            <span v-if="req.paymentMethod" class="px-2.5 py-1 rounded-full text-xs font-bold bg-white border border-neutral-200 text-neutral-600">
              付款：{{ req.paymentMethod }}
            </span>
            <span v-if="req.invoiceType" class="px-2.5 py-1 rounded-full text-xs font-bold bg-white border border-neutral-200 text-neutral-600">
              发票：{{ req.invoiceType }}
            </span>
            <span v-if="req.deliveryMethod" class="px-2.5 py-1 rounded-full text-xs font-bold bg-white border border-neutral-200 text-neutral-600">
    ...
    >
      <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-xs font-bold text-neutral-500 uppercase tracking-wider mb-2">意向价格（元/吨）</label>
            <input
          ...
          <div>
            <label class="block text-xs font-bold text-neutral-500 uppercase tracking-wider mb-2">交付地</label>
            <input
