...
async function loadPost() {
  ...
  try {
    ...
    if (r.code !== 0 || !r.data) {
      throw new Error(r.message || '文章不存在')
    }

    const post = r.data

    // 检查权限：只有作者可以编辑
    if (post.userId !== auth.me?.userId) {
      showToast.error('您没有权限编辑此文章')
      router.back()
      return
    }
    ...
    postType.value = (post.postType as PostType) || 'general'
    price.value = post.price || 9.9
    teaserLength.value = post.teaserLength || 100
    ...
    if (post.imagesJson) {
      try {
        ...
        if (Array.isArray(imgs)) {
          coverImages.value = imgs
        }
      } catch (e) { /* ignore */ }
    }
  } catch (e: any) {
    showToast.error(e?.message || '加载文章失败')
    router.back()
  } finally {
    loading.value = false
  }
...
async function submit() {
  ...
  try {
    const r = await updatePost(postId.value, {
    ...
    router.push(`/talks/${postId.value}`)
  } catch (e: any) {
  ...
  <div class="min-h-screen bg-white flex flex-col">
    ...
    <header class="h-14 border-b border-neutral-100 flex items-center justify-between px-6 bg-white sticky top-0 z-50">
      <div class="flex items-center gap-4">
        ...
        <h1 class="text-base font-bold text-neutral-900">编辑话题</h1>
      </div>
    ...
    <div v-if="loading" class="flex-1 flex items-center justify-center">
      <div class="text-center">
        <Loader2 :size="32" class="animate-spin text-brand-600 mx-auto mb-3" />
