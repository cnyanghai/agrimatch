      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          <div class="flex items-center gap-8">
            <span class="text-2xl font-bold text-blue-600 italic cursor-pointer" @click="go('/')">AgriMatch</span>
            <div class="hidden md:flex space-x-6 text-sm font-medium text-gray-600">
              <button class="hover:text-blue-600" @click="go('/')">首页</button>
              <button class="hover:text-blue-600" @click="go('/hall/supply')">供应大厅</button>
              <button class="text-blue-600 border-b-2 border-blue-600 pb-5" @click="go('/hall/need')">采购大厅</button>
              <button class="hover:text-blue-600" @click="go('/insights')">观点资讯</button>
              <button class="hover:text-blue-600" @click="go('/talks')">话题广场</button>
            </div>
          </div>
          <div class="flex items-center gap-3">
            <button class="text-sm text-gray-500 hover:text-blue-600" @click="openLogin">登录/注册</button>
            <button class="bg-blue-600 text-white px-5 py-2 rounded-lg font-bold text-sm hover:bg-blue-700 transition-colors" @click="onPublishNeed">
              发布采购
            </button>
          </div>
        </div>
