...
<template>
  <div class="bg-neutral-50 min-h-screen flex flex-col">
    <main class="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
      <!-- Header -->
      <div class="mb-12">
        <h1 class="text-2xl font-bold text-neutral-900 mb-4 flex items-center gap-3">
          <Truck v-if="type === 'supplier'" class="text-brand-600" :size="32" aria-hidden="true" />
          <ShoppingBag v-else class="text-action-600" :size="32" aria-hidden="true" />
          {{ type === 'supplier' ? '制造商名录' : '采购商名录' }}
        </h1>
        <p class="text-neutral-500">查找并对比优质{{ type === 'supplier' ? '饲料原料供应商' : '行业采购商' }}</p>
      </div>

      <!-- Search Bar -->
      <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6 mb-8">
        <div class="relative">
          <Search :size="20" class="absolute left-4 top-1/2 -translate-y-1/2 text-neutral-400" aria-hidden="true" />
          <input
            type="text"
            v-model="searchKeyword"
            @input="handleSearch(($event.target as HTMLInputElement).value)"
            placeholder="搜索公司名称..."
            class="w-full pl-12 pr-4 py-3 rounded-lg border border-neutral-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-200 outline-none transition-all text-sm"
            aria-label="搜索公司"
          />
        </div>
      </div>
      ...
      <div v-if="searchResults.length > 0 && searchKeyword" class="bg-white rounded-xl shadow-lg border border-neutral-200 mb-8 overflow-hidden">
        <div class="p-4 bg-neutral-50 border-b border-neutral-100">
          <div class="text-xs font-bold text-neutral-500 uppercase tracking-widest">搜索结果</div>
        </div>
        <div class="max-h-96 overflow-y-auto">
          <div
            ...
            class="flex items-center justify-between p-4 hover:bg-brand-50 cursor-pointer transition-colors border-b border-neutral-50 last:border-0"
          >
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="font-bold text-neutral-900">{{ result.companyName }}</div>
                <div class="text-xs text-neutral-400">{{ result.address || '暂无地址' }}</div>
              </div>
            </div>
            <ChevronRight :size="16" class="text-neutral-300" aria-hidden="true" />
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-8 mb-8">
        <div class="flex flex-wrap gap-2">
          <span class="text-xs font-bold text-neutral-400 uppercase tracking-widest mr-4 self-center">跳转到</span>
          <button
            class="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xs transition-all"
            :class="!letter ? 'bg-brand-600 text-white shadow-md shadow-brand-200' : 'bg-neutral-50 text-neutral-500 hover:bg-neutral-100'"
            @click="setLetter('')"
          >
            全部
          </button>
          <button
            ...
            :key="l"
            class="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-xs transition-all"
            :class="letter === l ? (type === 'supplier' ? 'bg-brand-600 text-white shadow-md shadow-brand-200' : 'bg-action-600 text-white shadow-md shadow-action-200') : 'bg-neutral-50 text-neutral-500 hover:bg-neutral-100'"
            @click="setLetter(l)"
          >
            {{ l }}
          </button>
      ...
      <div v-else-if="companies.length === 0" class="bg-white rounded-xl border border-dashed border-neutral-200">
        <EmptyState
          type="search"
          title="未找到相关公司"
