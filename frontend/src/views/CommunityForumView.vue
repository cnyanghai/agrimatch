...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-neutral-900">社区论坛</h1>
        <p class="text-sm text-neutral-500 mt-1">发布观点、点赞评论、与供需交流</p>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="bg-white rounded-xl border border-neutral-200 p-6">
        <h3 class="text-2xl font-bold text-neutral-900 mb-4">发布话题</h3>
        ...
        <div v-else class="space-y-4">
          <div class="flex items-center gap-3 p-3 bg-neutral-50 rounded-xl">
            ...
            <div class="flex-1 min-w-0">
              <div class="font-bold text-neutral-900 truncate">{{ auth.me?.nickName || auth.me?.userName || '-' }}</div>
              <div class="text-xs text-neutral-500 truncate">{{ company?.companyName ?? '未绑定公司' }}</div>
            </div>
          </div>

          <div>
            <label class="block text-xs font-bold text-neutral-500 uppercase tracking-wider mb-2">所属板块</label>
              ...
              <button
                ...
                :class="[
                  ...
                  form.domain === d.key
                    ...
                    : 'bg-neutral-50 text-neutral-600 border-neutral-200 hover:border-brand-500'
                ]"
          ...
          <div>
            <label class="block text-xs font-bold text-neutral-500 uppercase tracking-wider mb-2">话题内容</label>
            <textarea
      ...
      <!-- 帖子列表 -->
      <div class="lg:col-span-2 bg-white rounded-xl border border-neutral-200 overflow-hidden text-sm">
        <div class="p-4 border-b border-neutral-200 space-y-4">
          <div class="flex items-center justify-between gap-4">
            <h3 class="text-2xl font-bold text-neutral-900">话题列表</h3>
            <div class="relative flex-1 max-w-[300px]">
              ...
              <input
                ...
                type="text"
                placeholder="搜索话题..."
                class="w-full pl-10 pr-4 py-2 border-2 border-neutral-200 rounded-xl text-sm focus:border-brand-500 outline-none transition-all"
                @keyup.enter="refresh"
              />
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
            <span class="text-xs font-bold text-neutral-400 uppercase tracking-wider">板块:</span>
            <div class="flex flex-wrap gap-2">
              <button
                class="px-3 py-1 rounded-full text-xs font-bold transition-all border"
                :class="!q.domain ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-neutral-500 border-neutral-200 hover:border-brand-500'"
              ...
              <button
                ...
                :key="d.key"
                class="px-3 py-1 rounded-full text-xs font-bold transition-all border"
                :class="q.domain === d.key ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-neutral-500 border-neutral-200 hover:border-brand-500'"
        ...
        <div v-else class="divide-y divide-neutral-50">
          ...
          >
            <div class="flex items-start gap-4">
              <!-- 头像 -->
              <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-action-500 to-action-600 text-white flex items-center justify-center font-bold flex-shrink-0 overflow-hidden">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mb-1">
                  <span class="font-bold text-neutral-900">{{ post.nickName || post.userName || '匿名' }}</span>
                  <span v-if="post.domain && post.domain !== 'general'" class="bg-action-50 text-action-600 text-[10px] px-1.5 py-0.5 rounded font-bold">
                    {{ domains.find(d => d.key === post.domain)?.name || post.domain }}
                  </span>
                  <span v-if="post.companyName" class="text-xs text-neutral-400">· {{ post.companyName }}</span>
                  <span class="text-xs text-neutral-400 ml-auto">{{ formatTime(post.createTime) }}</span>
                </div>

                <!-- 标题 -->
                <h4 class="font-bold text-neutral-900 mb-2 line-clamp-2">{{ post.title }}</h4>

                <!-- 内容预览 -->
                <p v-if="post.content" class="text-sm text-neutral-600 line-clamp-2 mb-3">{{ post.content }}</p>
                ...
                <div class="flex items-center gap-3">
                  ...
                  <button
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-bold bg-neutral-100 text-neutral-600 hover:bg-action-50 hover:text-action-500 transition-all"
                  ...
                  <button
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-bold bg-neutral-100 text-neutral-600 hover:bg-warning-50 hover:text-warning-500 transition-all"
                  ...
                  <button
                    ...
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-bold bg-neutral-100 text-neutral-600 hover:bg-error-50 hover:text-error-500 transition-all ml-auto"
      ...
      <div class="space-y-4">
        ...
        <div class="flex gap-3">
          <textarea
            ...
            maxlength="1000"
            placeholder="输入评论..."
            class="flex-1 px-4 py-2.5 border-2 border-neutral-200 rounded-lg focus:border-brand-500 outline-none transition-all resize-none"
          ></textarea>
        ...
        <div class="border-t border-neutral-200 pt-4">
          ...
          <div v-else class="space-y-4 max-h-[400px] overflow-y-auto">
            <div
              ...
              class="flex gap-3"
            >
              <div class="w-8 h-8 rounded-lg bg-neutral-100 text-neutral-600 flex items-center justify-center text-xs font-bold flex-shrink-0">
                {{ (c.nickName || c.userName || '?')[0] }}
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                  <span class="font-bold text-neutral-900 text-sm">{{ c.nickName || c.userName || '匿名' }}</span>
                  <span v-if="c.companyName" class="text-xs text-brand-600 bg-brand-50 px-2 py-0.5 rounded-full">{{ c.companyName }}</span>
                  <span class="text-xs text-neutral-400 ml-auto">{{ formatTime(c.createTime) }}</span>
                </div>
    ...
    <BaseModal v-model="tipDialogOpen" title="积分打赏" size="sm">
      <div class="space-y-5">
        ...
        <div class="bg-neutral-50 rounded-xl p-4 border border-neutral-200">
          <div class="font-bold text-neutral-900 line-clamp-2 mb-3">{{ currentTipPost?.title }}</div>
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="font-bold text-neutral-700 truncate">{{ currentTipPost?.nickName || currentTipPost?.userName || '作者' }}</div>
              <div v-if="currentTipPost?.companyName" class="text-xs text-neutral-400 truncate">{{ currentTipPost?.companyName }}</div>
            </div>
          </div>
        </div>
        ...
        <!-- 积分数量 -->
        <div>
          <label class="block text-xs font-bold text-neutral-500 uppercase tracking-wider mb-2">打赏积分</label>
          <input
            v-model.number="tipForm.points"
            type="number"
            min="1"
            max="10000"
            class="w-full px-4 py-2.5 border-2 border-neutral-200 rounded-lg focus:border-brand-500 outline-none transition-all text-center text-lg font-bold"
          />
        </div>
        ...
        <div>
          <label class="block text-xs font-bold text-neutral-500 uppercase tracking-wider mb-2">
            打赏留言 <span class="text-neutral-400 font-normal">(可选)</span>
          </label>
          <input
