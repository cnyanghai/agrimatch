...
function getSystemMessageStyle(payload: SystemMessagePayload | null) {
  if (!payload) return { icon: 'info' as const, bgColor: 'bg-neutral-50', textColor: 'text-neutral-600', borderColor: 'border-neutral-200' }
  switch (payload.action) {
    ...
    case 'CONTRACT_CREATED': return { icon: 'check' as const, bgColor: 'bg-action-50', textColor: 'text-action-700', borderColor: 'border-action-200' }
    case 'QUOTE_ACCEPTED': return { icon: 'check' as const, bgColor: 'bg-green-50', textColor: 'text-green-700', borderColor: 'border-green-200' }
    case 'QUOTE_REJECTED': return { icon: 'alert' as const, bgColor: 'bg-error-50', textColor: 'text-error-600', borderColor: 'border-error-200' }
    default: return { icon: 'info' as const, bgColor: 'bg-neutral-50', textColor: 'text-neutral-600', borderColor: 'border-neutral-200' }
  }
}
...
<template>
  <div class="flex flex-col h-full">
    <!-- 聊天头部 -->
    <div class="px-3 py-2 border-b border-neutral-100 bg-neutral-50/50 shrink-0">
      <div class="flex items-center gap-2">
        <div class="relative">
          ...
          <div
            :class="['absolute bottom-0 right-0 w-2 h-2 rounded-full border-2 border-white', wsConnected ? 'bg-green-500' : 'bg-neutral-300']"
          />
        </div>
        <div>
              ...
              <div class="flex items-center gap-2">
                <button
                  ...
                  :class="[
                    ...
                    giftPointsAmount === amount ? 'bg-warning-500 text-white' : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'
                  ]"
                >
                  {{ amount }}
                </button>
