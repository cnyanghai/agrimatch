...
function getStatusColor(status?: QuoteStatus) {
  switch (status) {
    ...
    case 'EXPIRED': return 'bg-neutral-400 text-white'
    default: return 'bg-warning-500 text-white'
  }
}
...
<template>
  <div class="relative">
    ...
    >
      ...
      <div
        ...
        :class="[
          ...
          item.isSent ? 'border-brand-200 bg-brand-50/30' : 'border-neutral-200'
        ]"
      >
        <!-- 头部 -->
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-2">
            <span :class="['text-xs font-bold', item.isSent ? 'text-brand-600' : 'text-neutral-600']">
          ...
          <span class="text-[10px] text-neutral-400">{{ formatTime(item.time) }}</span>
        </div>

        <!-- 摘要 -->
        <div class="text-sm font-medium text-neutral-900">{{ item.summary }}</div>

        <!-- 价格变更 -->
        <div v-if="item.priceChange" class="mt-2 flex items-center gap-1 text-xs text-warning-600">
      ...
      >
        <svg class="w-2 h-4 text-neutral-300" viewBox="0 0 8 16" fill="currentColor">
    ...
    <!-- 空状态 -->
    <div v-if="timelineItems.length === 0" class="text-center py-8 text-neutral-400 text-sm">
      暂无报价记录
    </div>
  </div>
</template>
