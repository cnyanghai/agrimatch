...
const statusMap: Record<number, { label: string; color: string }> = {
  ...
  2: { label: '已签署', color: 'text-emerald-600 bg-emerald-50' },
  3: { label: '履约中', color: 'text-blue-600 bg-blue-50' },
  4: { label: '已完成', color: 'text-emerald-700 bg-emerald-100' },
...
<template>
  <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden max-w-[320px]">
    <!-- 头部 -->
      ...
      <div class="flex gap-2">
        <div class="flex-1 bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div class="flex items-center justify-center gap-1">
            <Check v-if="payload.buyerSigned" class="w-3.5 h-3.5 text-emerald-500" />
            <Clock v-else class="w-3.5 h-3.5 text-amber-500" />
            <span :class="['text-xs font-medium', payload.buyerSigned ? 'text-emerald-600' : 'text-amber-600']">
        ...
        <div class="flex-1 bg-gray-50 rounded-lg p-2 text-center">
          ...
          <div class="flex items-center justify-center gap-1">
            <Check v-if="payload.sellerSigned" class="w-3.5 h-3.5 text-emerald-500" />
            <Clock v-else class="w-3.5 h-3.5 text-amber-500" />
            <span :class="['text-xs font-medium', payload.sellerSigned ? 'text-emerald-600' : 'text-amber-600']">
    ...
    <div class="px-4 py-3 bg-gray-50 border-t border-gray-100 flex gap-2">
      <button
        class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-white border border-gray-200 rounded-xl text-xs font-bold text-gray-700 hover:bg-gray-50 transition-all active:scale-95"
      ...
      <button
        v-if="canSign"
        class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-600 rounded-xl text-xs font-bold text-white hover:bg-emerald-700 transition-all active:scale-95"
