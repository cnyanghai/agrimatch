...
function openAgreement(type: 'user' | 'privacy') {
  ...
  ElMessage.info(`${title}详情页面正在准备中...`)
}
</script>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
        ...
        <div>
          ...
          <ul class="space-y-4 text-xs font-bold">
            <li><button class="hover:text-brand-400 transition-colors" @click="openAgreement('user')">用户协议</button></li>
            <li><button class="hover:text-brand-400 transition-colors" @click="openAgreement('privacy')">隐私政策</button></li>
            <li><button class="hover:text-brand-400 transition-colors">电子签约法律效力</button></li>
            <li><button class="hover:text-brand-400 transition-colors">免责声明</button></li>
          </ul>
        </div>
        ...
        <div>
          ...
          <ul class="space-y-4 text-xs font-bold">
            <li><button class="hover:text-brand-400 transition-colors">商务合作</button></li>
            <li><button class="hover:text-brand-400 transition-colors">意见反馈</button></li>
            <li class="flex items-center gap-3">
              <span class="w-2 h-2 bg-brand-500 rounded-full animate-pulse"></span>
              <span class="text-gray-300">客服在线 (9:00-18:00)</span>
            </li>
            <li class="text-[10px] text-gray-600 font-medium">辽宁省大连市瓦房店市工业区</li>
          </ul>
        </div>
      </div>
